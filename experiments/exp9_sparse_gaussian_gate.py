"""
Experiment 9: SparseGaussianGate — 模拟神经元的离散感受野响应

用 N 个高斯的混合作为门控，模拟神经元的离散响应模式：
...
class MLP(nn.Module):
    def __init__(self, n_embd, activation='gelu'):
        ...
        if 'sparse' in activation:
            n_gauss = int(activation.split('_')[1]) if '_' in activation else 4
            self.act = SparseGaussianGate(n_gaussians=n_gauss, init_sigma=1.0, spread=2.0)
        elif activation == 'gaussian_gate':
            self.act = GaussianGate()
...
def train(model, loader, max_iters, device, lr=5e-4, warmup=100):
    ...
    # 分层学习率：Gaussian 类参数用 2x lr
    gate_params = [p for n, p in model.named_parameters()
                   if 'mu' in n or 'sigma' in n or 'w' in n]
    base_params  = [p for n, p in model.named_parameters()
                   if not any(x in n for x in ['mu', 'sigma', 'w'])]

    opt_args = [{'params': base_params,  'lr': lr}]
    if gate_params:
        opt_args.append({'params': gate_params,  'lr': lr * 2.0})

    optimizer = optim.AdamW(opt_args, weight_decay=0.1)
    model.train()
    best_loss = float('inf')
...
def main():
    print("=" * 70)
    print("Experiment 9: SparseGaussianGate — 模拟神经元的离散感受野")
    print("=" * 70)
    print("""
SparseGaussianGate: 用 N 个高斯模拟离散响应模式
...
参数对比：
- GaussianGate (N=1):  2 参数
- SparseGaussianGate (N=4): 12 参数
- SparseGaussianGate (N=8): 24 参数
    """)
    print("=" * 70)
    ...
    # 实验配置：N=1,4,8 加上 GELU 基准
    configs = [
        ('gelu',              'GELU (baseline)'),
        ('gaussian_gate',     'GaussianGate (N=1)'),
        ('sparse_gaussian_4', 'SparseGaussianGate (N=4)'),
        ('sparse_gaussian_8', 'SparseGaussianGate (N=8)'),
    ]

    results = {}

    for act_name, act_desc in configs:
        print(f"\n{'='*60}")
        print(f"Training: {act_desc}")
        print(f"{'='*60}")
        ...
        # 统计门控层
        n_sparse = sum(1 for m in model.modules() if isinstance(m, SparseGaussianGate))
        n_gate   = sum(1 for m in model.modules() if isinstance(m, GaussianGate))
        n_total  = n_sparse + n_gate

        print(f"Parameters: {n_params:,}")
        if n_total:
            print(f"Gaussian layers: {n_total}")
    ...
    print("=" * 70)
    print(f"{'Activation':<30} {'Train':>10} {'Test':>10} {'Time':>8}")
    print("-" * 60)
